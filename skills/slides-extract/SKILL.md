...
Use `uv run slides docs method:extract` to verify the current contract, then:

```bash
uv run slides extract <template_or_sample.pptx> \
...
Use `uv run slides docs schema:design-profile` for the full schema.
...
On any slides error, run `uv run slides docs method:extract` to verify the current contract before retrying.
